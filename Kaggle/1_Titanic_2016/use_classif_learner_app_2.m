...
best_trainedClassifier = cell(8, 1);
best_indices = [8 9 11 16 19 20 21 23];  % accuracy > 0.79
for idx = 1:8
    best_trainedClassifier{idx, 1} = trainedClassifier{best_indices(idx), 1};
end;
num_classifiers = 8;
...
for i = 1 : num_classifiers
    valid_preds(:, i) = best_trainedClassifier{i, 1}.predictFcn(valid_set(:, 2:10));
end;
...
for i = 1 : num_classifiers
    submit_preds(:, i) = best_trainedClassifier{i, 1}.predictFcn(test_matrix(:, 2 : 10));
end;
...
writetable(submit_table, 'submit_matlabclasslearnerapp_best8.csv');

toc   % Elapsed time is 126.530825 seconds.
