...
for i = 1 : 40
    disp(i);
	to_impute = all_clusters_relev_feats{i, 1};
    cluster_data = valid_clusters_relev_feats{i, 1};
    idx = knnsearch(to_impute(:, 2:end-1), cluster_data(:, 2:end-1));
    imputed_cluster = zeros(size(cluster_data));
    imputed_cluster(isnan(cluster_data) == 0) = cluster_data(isnan(cluster_data) == 0);
    imputed_cluster = imputed_cluster + to_impute(idx, :) .* isnan(cluster_data);
    if sum(sum(isnan(imputed_cluster))) > 0
		to_impute = all_balanced_clusters{i, 1};
    ...
    valid_clusters{i, 1} = imputed_cluster;
end;

save('valid_clusters_with_rel_feats_knn.mat', 'valid_clusters');
